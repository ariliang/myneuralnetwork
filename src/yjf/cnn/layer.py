...
class PoolLayer(Layer):
    ...
    def forward(self):
        if self.pool_method == "MAX":
            max_out, max_pos = util.pool_forward_max_argmax_tf_v(self.preLayer.A, self.pool_size, self.stride,
                                                                 self.padding)
            self.A = max_out
    ...
    def backward(self):
        if self.pool_method == "MAX":
            ...
            self.preLayer.dA = util.avg_pooling_backprop(self.preLayer.A.shape, self.pool_size, self.dA, self.stride,
                                                         self.padding)

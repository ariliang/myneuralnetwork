...
def pool_forward_max_argmax(mats, fsize, stride=1, padding="VALID"):
    ...
    h = out_shape[0]
    w = out_shape[1]
    mats2 = np.pad(mats, ((0, 0), (pad_h[0], pad_h[1]), (pad_w[0], pad_w[1]), (0, 0)), 'constant',
                   constant_values=(0, 0))
        ...
        for c in range(dZ.shape[3]):  # dZ的通道数
            for i in range(dZ.shape[1]):
                ...
                for j in range(dZ.shape[2]):
                    j_s = j * stride
                    dA2[m, i_s:i_s + f, j_s:j_s + f] = dA2[m, i_s:i_s + f, j_s:j_s + f] + dZ[m, i, j, c] * filters[:, :,
                                                                                                           :, c]
...
def conv_backprop_filter(inputmats, filter_sizes, out_backprop, stride, padding='VALID'):
    ...
    preA2 = np.pad(preA, ((0, 0), (pad_h[0], pad_h[1]), (pad_w[0], pad_w[1]), (0, 0)), 'constant',
                   constant_values=(0, 0))
